...
for i in range(0,decoupage-1):
	X.append(Xmin+(Xmax-Xmin)*(i+1)/float(decoupage))
X.append(Xmax)
...
def function_r(param_list):
	...
	strat={"strattype":"dichotomierapport"+str(param)}
...
tempgraph=custom_graph.CustomGraph(X,ymin=0,ymax=1,xmin=Xmin,xmax=Xmax)
tempgraph.empty()
...
tempgraph.std=1
tempgraph.title="Graph suggere par Jon, entropypop, dichotomierapport, M="+str(M)+" W="+str(W)+" N="+str(nbagents)+" iter="+str(nb_iter)
tempgraph.show()
